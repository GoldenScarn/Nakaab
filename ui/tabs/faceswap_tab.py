...
def faceswap_tab():
    ...
    custom_css = """
        .left-panel {
            background: linear-gradient(135deg, rgba(30, 30, 46, 0.95), rgba(49, 46, 129, 0.3));
            border-radius: 12px;
            padding: 16px;
            border: 1px solid rgba(139, 92, 246, 0.3);
        }
        .center-panel {
            background: linear-gradient(135deg, rgba(30, 30, 46, 0.95), rgba(88, 28, 135, 0.3));
            border-radius: 12px;
            padding: 20px;
            border: 1px solid rgba(139, 92, 246, 0.3);
        }
    ...
    with gr.Tab("🎭 Face Swap"):
        ...
        # gr.HTML("<div style='margin-top: -100rem;'></div>")
        with gr.Row():
            # ========== LEFT COLUMN - Face Management (Reduced Width) ==========
            with gr.Column(scale=3, elem_classes="left-panel"):
                gr.Markdown("### 🎯 Input Faces", elem_classes="section-header")
                ...
                with gr.Row():
                    bt_remove_selected_input_face = gr.Button("✕ Remove Selected", size='sm', scale=1)
                    bt_clear_input_faces = gr.Button("💥 Clear All", variant='stop', size='sm', scale=1)
                    bt_srcfiles = gr.Files(
                        label='Upload input faces',
                        file_count="multiple",
                ...
                gr.Markdown("### 👥 Target Faces", elem_classes="section-header")
                target_faces = gr.Gallery(
                    label="",
                    allow_preview=False,
                    columns=6,  # More columns for compact single row
                    object_fit="scale-down",
                    height=120,  # Reduced height for single row
                    interactive=False,
                    elem_classes="compact-target"
                )
                
                with gr.Row():
                    bt_remove_selected_target_face = gr.Button("✕ Remove Selected", size='sm', scale=1)
                    bt_add_local = gr.Button("📁 Add from Folder", size='sm', scale=1)
                    bt_destfiles = gr.Files(
                        label='Upload target faces/videos',
                        file_count="multiple",
                ...
                with gr.Row():
                    local_folder = gr.Textbox(show_label=False, placeholder="/path/to/folder", container=False)

            # ========== CENTER COLUMN - Preview & Main Controls ==========
            with gr.Column(scale=5, elem_classes="center-panel"):
                gr.Markdown("### 🖼️ Preview Window", elem_classes="section-header")
                
                with gr.Row():
                    fake_preview = gr.Checkbox(label="🎭 Face Swap Preview", value=False)
                    bt_refresh_preview = gr.Button("🔄 Refresh", variant='secondary', size='sm')
                    bt_use_face_from_preview = gr.Button("📸 Use Face from Frame", variant='primary', size='sm')
                ...
                gr.Markdown("### 🎬 Frame Control", elem_classes="section-header")
                preview_frame_num = gr.Slider(1, 1, value=1, label="Frame Number", info='0:00:00', step=1.0)
                
                with gr.Row():
                    text_frame_clip = gr.Markdown('**Processing range:** [0 - 0]')
                with gr.Row():
                    set_frame_start = gr.Button("⬅ Set as Start Frame", size='sm', scale=1)
                    set_frame_end = gr.Button("Set as End Frame ➡", size='sm', scale=1)

            # ========== RIGHT COLUMN - Advanced Settings (Expanded) ==========
            with gr.Column(scale=4, elem_classes="right-panel"):
                gr.Markdown("### 🚀 Action Controls", elem_classes="section-header")
                
                with gr.Row():
                    bt_start = gr.Button("▶ START PROCESSING", variant='primary', size='lg', scale=2, elem_classes="action-button")
                    bt_stop = gr.Button("⏹ STOP", variant='secondary', size='lg', scale=1, elem_classes="stop-button")
                    gr.Button("👁️ Open Output Folder",variant='secondary', size='lg', scale=1).click(
                ...
                with gr.Accordion(label="✨ Enhancement", open=False):
                    ui.globals.ui_selected_enhancer = gr.Dropdown(
                        ["None", "GFPGAN", "Restoreformer++"],
                        value="GFPGAN",  # DEFAULT SET TO GFPGAN
                    ...
                    ui.globals.ui_blend_ratio = gr.Slider(
                        0.0, 1.0, value=0.65,
                        label="Blend Ratio",
                        info="Original/Enhanced mix"
                    )

                # COLLAPSIBLE: Processing Configuration
                with gr.Accordion(label="⚙️ Processing Configuration", open=False):
                    with gr.Row():
                        ...
                        num_swap_steps = gr.Slider(
                            ...
                            step=1.0,
                            label="Swap Steps",
                            info="More steps = better likeness",
                            scale=1
                        )
                ...
                with gr.Accordion(label="🔧 Options", open=False):
                    ...
                    roop.globals.keep_frames = gr.Checkbox(label="💾 Keep extracted frames", value=False)
                    roop.globals.wait_after_extraction = gr.Checkbox(label="⏸️ Wait after extraction", value=False)

                # COLLAPSIBLE: Masking
                with gr.Accordion(label="🎭 Masking", open=False):
                    selected_mask_engine = gr.Dropdown(
                        ["None", "Clip2Seg", "DFL XSeg"],
                        value="Clip2Seg",  # DEFAULT SET TO CLIP2SEG
                        label="Masking Engine"
                    )
                    
                    clip_text = gr.Textbox(
                        label="Objects to mask (comma-separated)",
                        value="cup,hands,hair,banana,glasses,spectacles,phone,clothes,jewelry",
                        interactive=True,
                        lines=2
                    )
                    ...
                    with gr.Row():
                        bt_toggle_masking = gr.Button("🖌️ Toggle Manual Masking", size="sm", scale=1)
                        bt_preview_mask = gr.Button("👁️ Preview Mask", variant="secondary", size="sm", scale=1)

                gr.Markdown("### 📤 Output Files", elem_classes="section-header")
                with gr.Row():
                    resultfiles = gr.Files(label='', interactive=False, height=110)
                with gr.Row():
                    resultimage = gr.Image(type='filepath', label='Output', interactive=False, height=380)
                    resultvideo = gr.Video(label='Video Result', interactive=False, visible=False, height=380)

        # Face selection row (hidden by default)
        with gr.Row(visible=False) as dynamic_face_selection:
            with gr.Column(scale=2):
                face_selection = gr.Gallery(label="Detected faces", allow_preview=False, height=138, columns=8)
        ...
        # ========== EVENT HANDLERS ==========
        previewinputs = [
            ui.globals.ui_selected_swap_model, preview_frame_num, bt_destfiles, fake_preview,
            ui.globals.ui_selected_enhancer, selected_face_detection, max_face_distance,
            ui.globals.ui_blend_ratio, selected_mask_engine, clip_text, no_face_action,
            autorotate, maskimage, chk_showmaskoffsets, chk_restoreoriginalmouth,
            num_swap_steps, ui.globals.ui_upscale
        ]
        ...
        bt_use_face_from_preview.click(
            ...
            outputs=[dynamic_face_selection, face_selection, target_faces, selected_face_detection]
        )
        ...
        bt_preview_mask.click(
            fn=on_preview_mask,
            inputs=[ui.globals.ui_selected_swap_model, preview_frame_num, bt_destfiles, clip_text, selected_mask_engine],
        ...
        start_event = bt_start.click(
            fn=start_swap,
            inputs=[
                ui.globals.ui_selected_swap_model, output_method, ui.globals.ui_selected_enhancer,
                selected_face_detection, roop.globals.keep_frames, roop.globals.wait_after_extraction,
                roop.globals.skip_audio, max_face_distance, ui.globals.ui_blend_ratio,
                selected_mask_engine, clip_text, video_swapping_method, no_face_action,
                autorotate, chk_restoreoriginalmouth, num_swap_steps,
                ui.globals.ui_upscale, maskimage
            ],
...
def on_preview_frame_changed(swap_model, frame_num, files, fake_preview, enhancer, detection, face_distance, blend_ratio,
                              selected_mask_engine, clip_text, no_face_action, auto_rotate, maskimage, show_face_area, restore_original_mouth, num_steps, upsample):
...
def start_swap( swap_model, output_method, enhancer, detection, keep_frames, wait_after_extraction, skip_audio, face_distance, blend_ratio,
                selected_mask_engine, clip_text, processing_method, no_face_action, autorotate, restore_original_mouth, num_swap_steps, upsample, imagemask, progress=gr.Progress()):
